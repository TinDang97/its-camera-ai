...
def generate_proto_files():
    ...
    for proto_file in proto_files:
        ...
        try:
            ...
            print(
                "  ✗ Error: grpcio-tools not found. Install with: pip install grpcio-tools"
            )
            return False
    ...
    for py_file in output_dir.glob("*_pb2*.py"):
        ...
        if "import processed_frame_pb2" in content:
            content = content.replace(
                "import processed_frame_pb2", "from . import processed_frame_pb2"
            )

        if "import streaming_service_pb2" in content:
            content = content.replace(
                "import streaming_service_pb2", "from . import streaming_service_pb2"
            )
...
def check_dependencies():
    ...
    try:
        ...
        grpc_tools_spec = importlib.util.find_spec("grpc_tools")

        if grpc_spec is None or grpc_tools_spec is None:
            raise ImportError("grpc or grpc_tools not found")
