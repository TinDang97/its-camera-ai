...
class SystemHardening:
    ...
    async def perform_system_hardening(
        ...
        for check in self.hardening_checks:
            if level == HardeningLevel.BASIC and check.severity not in [
                "CRITICAL",
                "HIGH",
            ]:
        ...
        results["success_rate"] = (
            results["successful_checks"] / len(results["checks_performed"]) * 100
    ...
    async def _execute_hardening_check(self, check: HardeningCheck) -> bool:
        ...
        try:
            ...
            if check.verification_command:
                await self._run_command(check.verification_command)
            ...
            logger.error(
                "Failed to execute hardening check",
                check_id=check.check_id,
                error=str(e),
            )
            return False
...
class KubernetesHardening:
    ...
    async def harden_kubernetes_cluster(self, manifest_dir: Path) -> dict[str, Any]:
        ...
        results["completed_at"] = datetime.now().isoformat()
        logger.info(
            "Kubernetes hardening completed",
    ...
    async def _apply_kubernetes_hardening(
        self, check: HardeningCheck, manifest_dir: Path
    ) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to apply Kubernetes hardening",
                check_id=check.check_id,
                error=str(e),
            )
            return False
                ...
                "ingress": [
                    {
                        "from": [
                            {
                                "namespaceSelector": {
                                    "matchLabels": {"name": "its-camera-ai-system"}
                                }
                            }
                        ]
                    }
...
class ProductionHardeningOrchestrator:
    ...
    async def perform_complete_hardening(
        self,
        manifest_dir: Path,
        hardening_level: HardeningLevel = HardeningLevel.ENHANCED,
    ) -> dict[str, Any]:
        ...
        try:
            ...
            results["results"][
                "system"
            ] = await self.system_hardening.perform_system_hardening(hardening_level)

            # Container hardening
            logger.info("Starting container hardening")
            results["results"][
                "container"
            ] = await self.container_hardening.harden_container_images()

            # Kubernetes hardening
            logger.info("Starting Kubernetes hardening")
            results["results"][
                "kubernetes"
            ] = await self.kubernetes_hardening.harden_kubernetes_cluster(manifest_dir)

            # Network hardening
            logger.info("Starting network hardening")
            results["results"][
                "network"
            ] = await self.network_hardening.harden_network_security()
    ...
    async def _generate_hardening_report(
    ...
    def _analyze_hardening_categories(
        self, results: dict[str, Any]
        ...
        for category, result in results.get("results", {}).items():
            if isinstance(result, dict):
                categories[category] = {
                    ...
                    "total_checks": result.get("successful_checks", 0)
                    ...
                    "status": (
                        ...
                        if result.get("success_rate", 0) > 90
                            ...
                            "NEEDS_IMPROVEMENT"
                            if result.get("success_rate", 0) > 75
                            else "CRITICAL"
                        )
                    ),
                }
...
async def run_production_hardening(
    manifest_dir: str = "infrastructure/kubernetes",
) -> dict[str, Any]:
    ...
    logger.info(
        "Starting production security hardening", manifest_dir=str(manifest_path)
    )

    # Run complete hardening
    hardening_results = await production_hardening.perform_complete_hardening(
        manifest_path, HardeningLevel.ENHANCED
    )
