...
class TestHealthEndpoints:
    ...
    async def test_readiness_check_healthy(self, test_client: TestClient):
        ...
        with (
            patch("its_camera_ai.api.routers.health.get_db") as mock_get_db,
    ...
    async def test_detailed_health_check(self, test_client: TestClient):
        ...
        with (
            patch("its_camera_ai.api.routers.health._check_database_health") as mock_db,
            patch("its_camera_ai.api.routers.health._check_redis_health") as mock_redis,
            patch(
                "its_camera_ai.api.routers.health._check_ml_models_health"
            ) as mock_ml,
            patch("its_camera_ai.api.routers.health._check_disk_space") as mock_disk,
            patch(
                "its_camera_ai.api.routers.health._check_memory_usage"
            ) as mock_memory,
        ):
            ...
            assert all(
                check["status"] == "healthy" for check in data["checks"].values()
            )
