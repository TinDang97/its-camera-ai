...
from ..ml.ml_pipeline import (
    ...
    ModelVersion, ExperimentResult
)
from ..ml.production_monitoring import (
    ProductionDashboard, Alert, AlertSeverity, DriftType
)
...
class PrometheusMetricsCollector:
    ...
    def record_inference(self, model_version: str, camera_id: str, 
    ...
    def update_model_accuracy(self, model_version: str, camera_id: str, accuracy: float):
...
class GitIntegration:
    ...
    async def commit_model_version(self, model_version: ModelVersion, 
        ...
        try:
            ...
            # Git operations
            subprocess.run(["git", "add", str(model_dir)], 
    ...
    async def trigger_ci_pipeline(self, commit_hash: str, pipeline_config: Dict[str, Any]):
...
class CICDOrchestrator:
    ...
    async def trigger_model_pipeline(self, model_version: ModelVersion, 
        ...
        logger.info(f"Triggered CI/CD pipeline {pipeline_id} for model {model_version.version}")
        return pipeline_id
...
"""
    ...
    async def _test_stage(self, job: PipelineJob):
        ...
        try:
            ...
            if avg_inference_time < self.validation_thresholds["max_latency_ms"]:
                job.logs.append(f"✓ Performance test passed: {avg_inference_time:.1f}ms")
            else:
                raise ValueError(f"Performance test failed: {avg_inference_time:.1f}ms > {self.validation_thresholds['max_latency_ms']}ms")
    ...
    async def _validate_stage(self, job: PipelineJob):
        ...
        if validation_results["accuracy"] < self.validation_thresholds["min_accuracy"]:
            raise ValueError(f"Accuracy {validation_results['accuracy']:.3f} below threshold {self.validation_thresholds['min_accuracy']}")
        
        if validation_results["p95_latency_ms"] > self.validation_thresholds["max_latency_ms"]:
            raise ValueError(f"Latency {validation_results['p95_latency_ms']:.1f}ms above threshold {self.validation_thresholds['max_latency_ms']}ms")
        
        # Store validation results
        job.artifacts["validation_results"] = json.dumps(validation_results)
        
        job.logs.append(f"✓ Validation passed: accuracy={validation_results['accuracy']:.3f}, latency={validation_results['p95_latency_ms']:.1f}ms")
    ...
    async def _deploy_stage(self, job: PipelineJob):
        ...
        env_config = self.deployment_configs.get(job.environment, {})
        ...
        if job.environment == DeploymentEnvironment.STAGING:
            ...
            job.artifacts["deployment_url"] = f"https://staging-ml.example.com/v/{job.model_version}"
            ...
            job.artifacts["canary_url"] = f"https://canary-ml.example.com/v/{job.model_version}"
            ...
            job.artifacts["production_url"] = f"https://ml.example.com/v/{job.model_version}"
    ...
    def get_pipeline_status(self, pipeline_id: str = None) -> Dict[str, Any]:
        ...
        if pipeline_id:
            ...
            pipeline_jobs = [
                job for job in (list(self.active_jobs.values()) + self.job_history)
            ...
            return {
                ...
                "success_rate": sum(1 for job in self.job_history if job.success) / max(1, len(self.job_history)),
                "recent_jobs": [self._job_to_dict(job) for job in self.job_history[-10:]]
            }
    ...
    def _job_to_dict(self, job: PipelineJob) -> Dict[str, Any]:
        ...
        return {
            ...
            "logs": job.logs[-5:] if len(job.logs) > 5 else job.logs  # Last 5 log entries
        }
...
class MLOpsIntegration:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.monitoring_thresholds = [
            ...
            MetricThreshold("throughput_fps", 50, "greater_than", AlertSeverity.WARNING)
        ]
        
        # Integration with ML pipeline
        self.ml_pipeline: Optional[ProductionMLPipeline] = None
        ...
        async def monitored_predict(frame, frame_id, camera_id):
            ...
            try:
                ...
                self.prometheus_collector.record_inference(
                    model_version=self.ml_pipeline.current_production_model.version if self.ml_pipeline.current_production_model else "unknown",
                    camera_id=camera_id,
    ...
    async def deploy_model_version(self, model_version: ModelVersion, 
        ...
        logger.info(f"Started deployment pipeline {pipeline_id} for model {model_version.version}")
        return pipeline_id
    ...
    async def check_model_health(self, model_version: str) -> Dict[str, Any]:
        ...
        if self.ml_pipeline:
            ...
            health_metrics = {
                ...
                "inference_throughput": status["components"]["inference_engine"].get("throughput_fps", 0),
                "avg_latency_ms": status["components"]["inference_engine"].get("avg_latency_ms", 0),
            ...
            for threshold in self.monitoring_thresholds:
                metric_value = health_metrics.get(threshold.metric_name.replace("_", ""), 0)
            ...
            health_metrics["health_score"] = self._calculate_health_score(health_metrics, alerts)
    ...
    def get_mlops_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "prometheus_enabled": PROMETHEUS_AVAILABLE and self.prometheus_collector.metrics_initialized,
...
# Factory function
async def create_mlops_integration(config_path: Union[str, Path] = None) -> MLOpsIntegration:
