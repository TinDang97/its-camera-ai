...
class FederatedAggregator:
    ...
    def _federated_averaging(
        self, decrypted_updates: dict[str, dict], total_samples: int
    ) -> dict[str, torch.Tensor]:
    ...
    def _federated_proximal(
        ...
        for key in aggregated:
            if key in base_weights:
                final_weights[key] = (1 - proximal_weight) * aggregated[
                    key
...
class FederatedCoordinator:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.aggregator = FederatedAggregator(
            config.get("aggregation_method", "fedavg")
        )
    ...
    async def start_federated_training(self) -> bool:
        ...
        if len(self.registered_clients) < self.min_clients_per_round:
            logger.error(
                f"Need at least {self.min_clients_per_round} clients to start training"
            )
            return False
        ...
        logger.info(
            f"Started federated training with {len(self.registered_clients)} clients"
        )
        return True
    ...
    async def _federated_training_loop(self):
        ...
        try:
            for round_num in range(self.global_rounds):
                ...
                logger.info(
                    f"Starting federated round {self.current_round}/{self.global_rounds}"
                )
                ...
                if len(selected_clients) < self.min_clients_per_round:
                    logger.warning(
                        f"Not enough clients available for round {self.current_round}"
                    )
    ...
    async def _select_clients_for_round(
        self, fed_round: FederatedRound
    ) -> list[ClientInfo]:
        ...
        available_clients = [
            ...
            if (
                ...
                and time.time() - client.last_seen < 300  # Last seen within 5 minutes
                and client.data_samples > 0
            )
        ]
    ...
    async def _coordinate_client_training(
        self, client_id: str, fed_round: FederatedRound
    ) -> bool:
    ...
    def _simulate_client_update(
        ...
        try:
            ...
            # Encrypt weights
            encrypted_weights = self.aggregator.security.encrypt_model_weights(
                mock_weights
            )
    ...
    async def _update_global_model(self, fed_round: FederatedRound):
        ...
        try:
            ...
            aggregated_weights = await self.aggregator.aggregate_updates(
                fed_round.client_updates, self.global_weights
            )
            ...
            if aggregated_weights:
                ...
                logger.info(
                    f"Updated global model to version {self.global_model_version}"
                )
    ...
    async def _check_convergence(self) -> bool:
        ...
        recent_accuracies = [
            round_data["accuracy"] for round_data in self.training_history[-5:]
        ]
        ...
        if len(recent_accuracies) >= 5:
            ...
            if converged:
                logger.info(
                    f"Convergence detected: avg improvement {avg_improvement:.4f} < {self.convergence_threshold}"
                )
    ...
    def _record_round_history(self, fed_round: FederatedRound):
        ...
        if fed_round.client_updates:
            avg_accuracy = np.mean(
                [
                    ...
                    for update in fed_round.client_updates.values()
                ]
            )
            avg_loss = np.mean(
        ...
        round_data = {
            ...
            "loss": avg_loss,
            "duration_minutes": (
                (fed_round.end_time - fed_round.start_time) / 60
                if fed_round.end_time
                else 0
            ),
    ...
    def get_training_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "current_round_participants": (
                len(active_round.participating_clients) if active_round else 0
            ),
            "training_history_length": len(self.training_history),
            "latest_accuracy": (
                self.training_history[-1]["accuracy"] if self.training_history else 0.0
            ),
        }
    ...
    async def stop_training(self):
        ...
        for _round_id, fed_round in self.active_rounds.items():
            if fed_round.status in [
                FederatedRoundStatus.TRAINING,
                FederatedRoundStatus.AGGREGATING,
            ]:
                fed_round.status = FederatedRoundStatus.COMPLETED
                fed_round.end_time = time.time()
...
# Factory function
async def create_federated_coordinator(
