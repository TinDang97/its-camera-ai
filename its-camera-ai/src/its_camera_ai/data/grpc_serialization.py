...
class ProcessedFrameSerializer:
    ...
    def __init__(
        ...
        self.compressor = ImageCompressor(
            default_format=compression_format,
            default_quality=compression_quality
        )
    ...
    def serialize_processed_frame(self, frame: 'ProcessedFrame') -> bytes:
        ...
        try:
            ...
            pb_frame.processing_stage = self._convert_processing_stage(frame.processing_stage)
    ...
    def _serialize_image_data(self, image: np.ndarray, image_type: str) -> 'pb.ImageData':
    ...
    def deserialize_processed_frame(self, serialized_data: bytes) -> 'ProcessedFrame':
        ...
        try:
            ...
            from its_camera_ai.data.streaming_processor import ProcessedFrame, ProcessingStage
            ...
            frame.processing_stage = self._convert_processing_stage_from_pb(pb_frame.processing_stage)
    ...
    def _deserialize_image_data(self, image_data: 'pb.ImageData') -> Optional[np.ndarray]:
        ...
        try:
            if image_data.compression_format == "raw":
                ...
                return np.frombuffer(image_data.compressed_data, dtype=np.uint8).reshape(shape)
            
            else:
                # Compressed data - decompress
                target_shape = (image_data.height, image_data.width, image_data.channels)
                return self.compressor.decompress_image(image_data.compressed_data, target_shape)
    ...
    def get_performance_metrics(self) -> dict[str, Any]:
        ...
        return {
            "avg_serialization_time_ms": sum(self.serialization_times) / len(self.serialization_times),
